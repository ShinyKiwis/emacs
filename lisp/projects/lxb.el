(defun my/start-lxb-perspective ()
  ...
  (let* ((persp-name "lxb")
    ...
    (unless existing
      ...
      (find-file "README.md")
      (dolist (spec
               '(("*rails-server*" . "bundle exec rails server -p 3001")
...
(defun my/setup-lxb-buffer-keys ()
  (when (string= (persp-name (persp-curr)) "lxb")
    ...
    (local-set-key (kbd "M-2") (lambda () (interactive) (switch-to-buffer "*webpack*")))
    (local-set-key (kbd "M-3") (lambda () (interactive) (switch-to-buffer "*rails-console*")))
